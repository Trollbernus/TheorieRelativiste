	...
	\section{Description géométrique relativiste du problème à $N$ corps}
		...
		\subsection{Coordonnées spatiales cartésiennes, contraintes sur les paramètres $z-e-\xi$}
			...
			\begin{equation}
				T^{\mu\nu}=O(c^2,c^1,c^0).
			\end{equation}
			À cause du facteur $8\pi\mathscr{G}/c^4$ de l'équation d'Einstein, nous avons :
			\begin{equation}
			...
			\end{equation}
			où $\mathrm{det}_3$ désigne le déterminant en dimension 3 et $\det_4$ le déterminant en dimension 4.
			...
			\end{eqnarray}
			Dans \ref{calcul_int}, nous utilisons les conditions d'isotropie spatiale pour simplifier la première ligne. Notons que d'après les hypothèse 1-PN sur le tenseur métrique et sur la matrice jacobienne, la deuxième ligne peut se simplifier si l'on néglige l'ordre $O(4)$. De telle sorte que nous avons 
			\begin{eqnarray}
			...
			Cette équation va nous permettre de caractériser précisément les $e^i_{\ j}$. En effet, tenant compte des résultats du paragraphe précédent et en négligeant les termes d'ordre $O(4)$, nous obtenons
			\begin{equation}
			...
			\begin{equation}
				e^j_{\ a} \left( 1-\frac{\bm{v}^2}{2c^2} \right) - \frac{v^i v^j}{2c^2} e^i_{\ a} = e^i_{\ a}\left( \delta^{ji}-\delta^{ji}\frac{\bm{v}^2}{2c^2}  - \frac{v^jv^i}{2c^2} \right) = e^i_{\ a}\left( 1-\frac{\bm{v}^2}{2c^2} \right)\left( \delta^{ji} - \frac{v^jv^i}{2c^2} \right) + O(4)
			\end{equation} 
			Autrement dit
			\begin{equation}
				e^0_{\ 0}e^i_{\ a}\left( 1-\frac{\bm{v}^2}{2c^2} \right)\left( \delta^{ji} - \frac{v^jv^i}{2c^2} \right)=R^j_{\ a}+O(4)
			\end{equation}
			Notons que $(1-\bm{v}^2}/2c^2)^{-1}=1+\bm{v}^2/2c^2+O(4)$ et que l'inverse de la matrice dont les composantes $ji$ sont $\delta^{ji} - v^jv^i/2c^2$ est la matrice dont les composantes $ij$ sont $\delta^{ij} + v^iv^j/2c^2 + O(4)$. Nous obtenons finalement
			\begin{equation}
				e^0_{\ 0}e^i_{\ a}=\left( 1+\frac{\bm{v}^2}{2c^2} \right)\left( \delta^{ij}+\frac{v^iv^j}{2c^2} \right)R^j_{\ a} + O(4).
			\end{equation}

			À présent nous voulons obtenir des contraintes sur $\xi^i$. Pour ce faire, nous allons analyser l'ordre $O(X)$ de l'équation \ref{contrainte_isotropie}.
			Cela s'écrit
			\begin{equation}
